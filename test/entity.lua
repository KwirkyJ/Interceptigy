...
TestManipTracks.test_getRealTrack_multipiece_built = function(self)
    local fx_app = trackfactory.tangent(self.curve_a, 42)
    local fy_app = trackfactory.tangent(self.curve_b, 42)
...
TestManipTracks.test_getProjectedTrack_curve_extrapolate = function(self)
    ...
    assertEquals(a:getDegree(t), 2, 'curve is quadratic')
    local exp_x = piecewise.Polynomial(t, a:getCoefficients(t))
    local exp_y = piecewise.Polynomial(t, b:getCoefficients(t))
...
TestManipTracks.test_getProjectedTrack_multipiece = function(self)
    ...
    local curve_bb = piecewise.insertPoly(self.curve_b, fy_app)
    self.e:setTrack(curve_aa, curve_bb)
    exp_x = trackfactory.tangent(self.curve_a, 33)
    exp_y = trackfactory.tangent(self.curve_b, 33)
    assertEquals({self.e:getProjectedTrack(33)}, {exp_x, exp_y})
    
    exp_x = piecewise.Polynomial(34, self.curve_a:getCoefficients(34))
    exp_y = piecewise.Polynomial(34, self.curve_b:getCoefficients(34))
    assertEquals({self.e:getProjectedTrack(34, 'extrapolate')}, {exp_x, exp_y})
    
    exp_x = piecewise.Polynomial(134, fx_app:getCoefficients(134))
    exp_y = piecewise.Polynomial(134, fy_app:getCoefficients(134))
